...
if __name__ == "__main__":
        ...
        #     if data_dict.get(x) < data.shape[0] / 20 else x)
        ...
        classifier = LinearSVC( C = 1, class_weight='balanced', random_state=0, verbose = True)
        # Build model
        ...
        # Run predictor
        predictions = pandas.DataFrame(loaded_model.predict(x_test))
        id_test = id_test.reset_index(drop=True)
        y_test = y_test.reset_index(drop=True)
        protocol = pandas.concat([id_test, predictions, y_test], axis=1)
        protocol = protocol.sort_values(by=['issueId'])
        # USE THIS AFTER ADDING THE JIRA ID TO DATASOURCE protocol = protocol.sort_values(by=['issueId'])
        protocol.to_csv(path_or_buf=prot_file, sep=';', index=False, quoting = csv.QUOTE_NONNUMERIC)

        accuracy = metrics.accuracy_score(
            y_test.fillna('0'), predictions.fillna('0'))
